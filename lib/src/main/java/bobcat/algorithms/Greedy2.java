    ...
    public double solve() {
        ...
        for (int i = 0; i < network.relayList.length; i++) {
            ...
            for (int k = 0; k < Network.thetaSet.length; k++) {
                ...
                for (int l = 0; l < network.beamSet[i][k].length; l++) {
                    ...
                    for (int n = 0; n < vv.length; n++) {
                        if (vv[n].getValue() > 0 && allocatedChannels < network.numChannels) {
                            //vv[n].sub.preferredRelay = r;
                            allocatedChannels++;
                            beamSetValue += vv[n].getValue();
                        }
                    }
            ...
            for (int n = 0; n < vv.length; n++) {
                ...
                if (vv[n].getValue() > 0 && r.allocatedChannels < network.numChannels) {
                    if (vv[n].getVertex().preferredRelay != null) {
                        vv[n].getVertex().preferredRelay.allocatedChannels--;
                    }
                    vv[n].getVertex().preferredRelay = r;
        ...
        for (int j = 0; j < network.subList.length; j++) {
            ...
            if (s.preferredRelay == null) {
                ...
                Arrays.sort(vv);
                for (int i = 0; i < network.relayList.length; i++) {
                    if (vv[i].getValue() > 0.0 && vv[i].getVertex().allocatedChannels < network.numChannels) {
                        s.preferredRelay = vv[i].getVertex();
                        vv[i].getVertex().allocatedChannels++;
                        break;
                    }
                }
            }
