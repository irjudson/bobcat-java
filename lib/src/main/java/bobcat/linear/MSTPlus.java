package bobcat.linear;

import bobcat.algorithms.MinimumSpanningTree;
import bobcat.network.Throughput;
import bobcat.simulation.ListElement;
import bobcat.simulation.Utilities;
import bobcat.simulation.Vertex;
...
public class MSTPlus {
    ...
    // Constructor
    public MSTPlus(int nodeNumber, int seed, double squareSide, int beams) {
    ...
    public void run() {
        ...
        // NEW PHASE 1 - use MST soln as starting point (Oct 5, 2011, Brendan):
        MinimumSpanningTree mst = new MinimumSpanningTree(nodeNumber, seed, squareSide, beams);
        mst.buildMinimumSpanningTree();
        mst.constructNewGraph();
        Vertex[] mstGraph = mst.getNewGraph();
        for (int i = 0; i < nodeNumber; i++) {
            for (int j = 0; j < beams + 1; j++) {
                if (mstGraph[i].activeBeams[j]) {
                    sectorUsed[i][j] = 1;
                }
            }
        }
...
//                    System.out.println("Brendan total Weight = " + totalWeight + " + " + weightItoJ + " + " + weightJtoI);
                    ...
                    Logger.getLogger(SSPLPR.class.getName()).log(Level.SEVERE, null, ex);
                } catch (IloException ex) {
                    Logger.getLogger(SSPLPR.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
